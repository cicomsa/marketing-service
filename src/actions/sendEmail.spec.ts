...
describe('sendEmail action', () => {
  ...
  it('should log the appropriate messages when the email is sent successfully', async () => {
    ...
    expect(console.log).toHaveBeenNthCalledWith(1, `awaiting ${delayTime} miliseconds`);
    expect(console.log).toHaveBeenNthCalledWith(2, `sending email to ${userEmail}`);
    expect(console.log).toHaveBeenNthCalledWith(3, `email was succesfully sent to user with email ${userEmail}`);
  });
  ...
  it('should log the appropriate messages when the email fails to be sent', async () => {
    ...
    expect(console.log).toHaveBeenNthCalledWith(1, `awaiting ${delayTime} miliseconds`);
    expect(console.log).toHaveBeenNthCalledWith(2, `sending email to ${userEmail}`);
    expect(console.log).toHaveBeenNthCalledWith(3, `email failed to be sent to user with email ${userEmail}`);
  });
});
