...
describe("GET /events", () => {
  ...
  describe("when userEmail and eventName are present in the query", () => {
    ...
    it("should return 200 and call the sendEmail adapter when event is email trigger", async () => {
      ...
      (getEmailConfig as jest.Mock).mockReturnValue([{ delayTime: 2000 }, { delayTime: 1000 }]);
      ...
      expect(mockSendEmail).toHaveBeenCalledTimes(mockEmailsConfig[eventName].length);
      // expect(sendEmail).toHaveBeenNthCalledWith(1, mockSend, mockEmailsConfig[eventName][0].delayTime, userEmail);
      expect(mockSendEmail).toHaveBeenLastCalledWith(send as jest.Mock, mockEmailsConfig[eventName][0].delayTime, userEmail);
    });
  });
});
